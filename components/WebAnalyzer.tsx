...
export const WebAnalyzer: React.FC<WebAnalyzerProps> = ({ onAuditComplete, language, onNotify }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const normalized = new URL(cleanUrl).toString();
      const report = await analyzeWebsite(normalized, language);

      const lead: Lead = {
        id: `audit-${Date.now()}`,
        name: normalized.replace('https://', '').replace('http://', '').replace('www.', '').split('/')[0],
  ...
  return (
    <div className="max-w-2xl mx-auto mt-20">
      <div className="bg-zinc-900/50 backdrop-blur-2xl p-10 rounded-3xl shadow-2xl border border-white/5 text-center relative overflow-hidden">
        ...
        <p className="text-zinc-500 mb-10 text-sm font-light relative z-10">{t.auditDesc}</p>
        ...
        <div className="flex flex-col items-center relative z-10">
          <Button onClick={handleAnalyze} disabled={!url} isLoading={isAnalyzing} className="w-full py-4 text-base">
            {t.analyzeBtn}
          </Button>
